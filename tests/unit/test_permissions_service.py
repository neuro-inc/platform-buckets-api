...
class TestPermissionsService:
    ...
    async def test_checker_1(
        ...
        assert not checker.can_read(fake_bucket)
        assert not checker.can_write(fake_bucket)
        assert not checker.can_read_any()
        assert not checker.can_write_any()
        assert checker.write_access_for_owner_by() == [username]
        assert checker.read_access_for_owner_by() == [username]

    async def test_checker_2(
        self,
        cluster_name: str,
        mock_auth_client: MockAuthClient,
        service: PermissionsService,
        ...
        checker = await service.get_perms_checker(fake_bucket.owner)
        assert checker.can_read_any()
        assert not checker.can_write_any()

    async def test_checker_3(
        self,
        cluster_name: str,
        mock_auth_client: MockAuthClient,
        service: PermissionsService,
